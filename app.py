...
# Pomocná funkce na odstranění diakritiky pro bezpečné PDF
...
# ----------------------------------------------------------------------
# Generování PDF protokolu (bezpečné pro české znaky)
# ----------------------------------------------------------------------
st.divider()
...
class PDFReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, remove_accents("Protokol navrhu plynovych vzper vika"), 0, 1, "C")
        self.ln(5)

    def footer(self):
        self.set_y(-15)
        self.set_font("Helvetica", "I", 8)
        self.cell(0, 10, remove_accents(f"Strana {self.page_no()}"), 0, 0, "C")
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    pdf.cell(0, 8, remove_accents("1. Zadane parametry"), 0, 1)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, remove_accents(f"Rezim: {app_mode}"), 0, 1)
    pdf.cell(0, 6, remove_accents(f"Rozmery vika: Delka = {lid_length} mm, Vyska = {lid_height} mm, Hmotnost = {lid_mass} kg"), 0, 1)
    pdf.cell(0, 6, remove_accents(f"Teziste (X, Y): {cg_x_mm} mm, {cg_y_mm} mm | Madlo (X, Y): {handle_x_mm} mm, {handle_y_mm} mm"), 0, 1)
    pdf.cell(0, 6, remove_accents(f"Max. uhel otevreni: {theta_max_deg} deg | Usporadani: {config_type} ({strut_type_key})"), 0, 1)
    pdf.ln(4)

    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, remove_accents("2. Vysledky vypoctu a pozice cepu"), 0, 1)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, remove_accents(f"Hlavni vzpera – Vana (X, Y): {Xb1} mm, {Yb1} mm | Cep na viku (X, Y): {lx1:.1f} mm, {ly1:.1f} mm"), 0, 1)
    pdf.cell(0, 6, remove_accents(f"Jmenovita sila hlavni vzpery (1 ks): {F_main:.0f} N"), 0, 1)
    if use_aux:
        pdf.cell(0, 6, remove_accents(f"Pomocna vzpera – Vana (X, Y): {Xb2} mm, {Yb2} mm | Cep na viku (X, Y): {lx2:.1f} mm, {ly2:.1f} mm"), 0, 1)
        pdf.cell(0, 6, remove_accents(f"Jmenovita sila pomocne vzpery (1 ks): {F_aux:.0f} N"), 0, 1)
    
    pdf.cell(0, 6, remove_accents(f"Sila na madlu k otevreni (@0°): {F_hand(0.0):.1f} N"), 0, 1)
    pdf.cell(0, 6, remove_accents(f"Sila na madlu k zavreni (@max): {f_max_val:.1f} N"), 0, 1)
    if theta_dead is not None:
        pdf.cell(0, 6, remove_accents(f"Mrtvy bod: {np.degrees(theta_dead):.1f}°"), 0, 1)
    pdf.ln(4)

    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, remove_accents("3. Graficke znazorneni"), 0, 1)
...
st.download_button(
    label="📥 Stáhnout kompletní protokol (PDF)",
    data=pdf_bytes,
